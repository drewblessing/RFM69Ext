...
// Adapted by Drew Blessing based on original work by felix@lowpowerlab.com, http://www.LowPowerLab.com
// **********************************************************************************************************
...
byte readSerialLine(char* input, int *nodeId, char untilChar='_', byte maxLength=64, uint16_t timeout=50);
...
void loop() {
  ...
  if (inputLen >= 6)
  {
    ...
    {
      Serial.print("OPN ... ");
      if (radio.sendWithRetry(nodeId, "OPN", 3))
        Serial.println("ok ... ");
    ...
    {
      Serial.print("CLS ... ");
      if (radio.sendWithRetry(nodeId, "CLS", 3))
        Serial.println("ok ... ");
      else Serial.println("nothing ... ");
    }
...
byte readSerialLine(char* input, int *nodeId, char untilChar, byte maxLength, uint16_t timeout)
{
  int inputLen = 0;
  Serial.setTimeout(timeout);
  inputLen = Serial.readBytesUntil(untilChar, input, maxLength);
